...
func Project(ctx context.Context, client *ent.Client) {
	...
	createdBy := seedutil.GetTeammateByEmail(ctx, client, teammateFeed.manato.Email)
	ws := seedutil.GetWorkspace(ctx, client)
	ts := []ent.CreateProjectInput{
		{
			Name:                projectFeed.appDevelopment.name,
			WorkspaceID:         ws.ID,
			ProjectBaseColorID:  seedutil.GetProjectBaseColorByColor(ctx, client, colorFeed.pink400.Color).ID,
			ProjectLightColorID: seedutil.GetProjectLightColorByColor(ctx, client, colorFeed.pink200.Color).ID,
			ProjectIconID:       seedutil.GetProjectIconByIcon(ctx, client, iconFeed.sun.Icon).ID,
		...
		},
		{
			...
			WorkspaceID:         ws.ID,
			ProjectBaseColorID:  seedutil.GetProjectBaseColorByColor(ctx, client, colorFeed.teal400.Color).ID,
			ProjectLightColorID: seedutil.GetProjectLightColorByColor(ctx, client, colorFeed.teal200.Color).ID,
			ProjectIconID:       seedutil.GetProjectIconByIcon(ctx, client, iconFeed.moon.Icon).ID,
		...
		},
		{
			...
			WorkspaceID:         ws.ID,
			ProjectBaseColorID:  seedutil.GetProjectBaseColorByColor(ctx, client, colorFeed.orange400.Color).ID,
			ProjectLightColorID: seedutil.GetProjectLightColorByColor(ctx, client, colorFeed.orange200.Color).ID,
			ProjectIconID:       seedutil.GetProjectIconByIcon(ctx, client, iconFeed.moon.Icon).ID,
