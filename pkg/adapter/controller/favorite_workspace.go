...
func (c *favoriteWorkspaceController) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.FavoriteWorkspaceWhereInput, requestedFields []string) (*model.FavoriteWorkspaceConnection, error) {
	return c.favoriteWorkspaceUsecase.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
