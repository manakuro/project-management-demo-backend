...
func (c *taskController) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.TaskWhereInput, requestedFields []string) (*model.TaskConnection, error) {
	return c.taskUsecase.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
