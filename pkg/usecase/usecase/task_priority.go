...
func (u *taskPriorityUsecase) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.TaskPriorityWhereInput, requestedFields []string) (*model.TaskPriorityConnection, error) {
	return u.taskPriorityRepository.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
