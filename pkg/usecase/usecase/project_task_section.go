...
func (u *projectTaskSectionUsecase) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.ProjectTaskSectionWhereInput, requestedFields []string) (*model.ProjectTaskSectionConnection, error) {
	return u.projectTaskSectionRepository.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
