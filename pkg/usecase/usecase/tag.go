...
func (u *tagUsecase) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.TagWhereInput, requestedFields []string) (*model.TagConnection, error) {
	return u.tagRepository.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
