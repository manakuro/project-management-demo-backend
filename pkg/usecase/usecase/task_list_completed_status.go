...
func (u *taskListCompletedStatusUsecase) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.TaskListCompletedStatusWhereInput, requestedFields []string) (*model.TaskListCompletedStatusConnection, error) {
	return u.taskListCompletedStatusRepository.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
