...
func (u *taskTagUsecase) ListWithPagination(ctx context.Context, after *model.Cursor, first *int, before *model.Cursor, last *int, where *model.TaskTagWhereInput, requestedFields []string) (*model.TaskTagConnection, error) {
	return u.taskTagRepository.ListWithPagination(ctx, after, first, before, last, where, requestedFields)
}
